...
from app.models import Post, PostMedia, PostType
from app import db
...
def _is_admin():
    return current_user.is_authenticated and (
        getattr(current_user, "is_staff", False) or
        getattr(current_user, "is_admin", False) or
        getattr(current_user, "role", "") in ("admin", "staff")
    )
...
def public_updates():
    ...
    return render_template("public_updates.html",
        results=results, discounts=discounts, videos=videos, pictures=pictures)
...
def _save_media_files(post, files):
    ...
    for f in files:
        ...
        folder = f"content/{post.id}/{media_type}s"
